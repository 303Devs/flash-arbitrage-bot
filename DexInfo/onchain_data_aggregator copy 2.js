...
const createFinalDataStructure = (chainData, tokenLookup) => {
  ...
  for (const dex of chainData.dexes) {
    ...
    for (const pool of dex.pools) {
      ...
      const poolData = {
        ...
        tradingMetrics: calculateTradingMetrics(pool, reserveUSD, availableLiquidityUSD)
      };
  ...
  // Create trading pairs (only pairs with multiple DEXes)
  ...
  // After creating pairGroups, update competitor pools count
  for (const [pairKey, pairData] of pairGroups) {
    for (const poolAddress of pairData.pools) {
      if (finalData.pools[poolAddress]) {
        finalData.pools[poolAddress].tradingMetrics.competitorPools = pairData.pools.length - 1; // Exclude self
      }
    }
  }
 ...
 */
const calculateTradingMetrics = (pool, reserveUSD, availableLiquidityUSD) => {
...
const calculatePairTradingScore = (pairData) => {
  ...
  // More DEXes = better trading opportunities
  ...
  const summary = {
    ...
    topTokensByPoolCount: Object.entries(finalData.tokens)
      .map(([address, tokenData]) => {
        // Count pools where this token appears (iterate through pools directly)
        const poolCount = Object.values(finalData.pools).filter(pool => 
