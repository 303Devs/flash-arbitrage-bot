...
function loadChainConfig(): ChainConfig {
  ...
  // Get Arbitrum config (chainId 42161)
  const arbitrumConfig = config['42161'];
  if (!arbitrumConfig) {
    throw new Error('Arbitrum config (42161) not found in chains.json');
  }
...
async function testRpcConnection(name: string, url: string, isWebSocket: boolean = false): Promise<boolean> {
  try {
    ...
    if (chainId !== 42161) {
      console.error(`❌ ${name}: Wrong chain ID. Expected 42161, got ${chainId}`);
      return false;
    }
...
async function runTests() {
  console.log('🧪 ARBITRUM CONFIG TEST STARTING...\n');
  ...
  try {
    ...
    if (passedTests === totalTests) {
      console.log(`\n🎉 ALL TESTS PASSED! Arbitrum config is ready for use.`);
    } else {
